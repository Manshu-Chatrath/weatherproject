...
const App = () => {
  ...
  useEffect(() => {
    (async () => {
      const response = await openWeatherApi.post(
        `onecall?lat=${latLongValues.lat}&lon=${latLongValues.long}&exclude=hourly,minutely&appid=${openWeatherApiKey}&units=metric`
      );
