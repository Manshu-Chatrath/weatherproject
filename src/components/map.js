...
const MapContainer = () => {
  ...
  const findlatlong = async () => {
    ...
    setLatLongValues({
      ...
      long: location.data.results[0].geometry.location.lng,
    });
    setCity(city);
    history.push("/");
  };
      ...
      <div className="location-container2">
        ...
        <div className="input">
          <div className="locationinput">
            <label className="locationlabel">Enter the location</label>
            <br />
            <input
              onChange={(e) => (city = e.target.value)}
              placeholder="Enter your city"
            />
            <button onClick={findlatlong} className="locationButton">
              Change Location
            </button>
          </div>
        </div>
        <div className="mapContainer">
          <div className="map">
            <GoogleMapReact
              ...
              defaultCenter={{
                lat: latLongValues.lat,
                lng: latLongValues.long,
              }}
              defaultZoom={6}
